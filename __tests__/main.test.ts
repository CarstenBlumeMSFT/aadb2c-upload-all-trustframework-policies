...
test('Upload Policies Client Secret', () => {
  ...
  delete process.env['INPUT_CLIENTCERTIFICATETHUMBPRINT']
  delete process.env['INPUT_CLIENTCERTIFICATEKEY']
  delete process.env['INPUT_CLIENTCERTIFICATEPASS']
...
test('Upload Policies Client Certificate', () => {
  ...
  process.env['INPUT_VERBOSE'] = 'true'
  delete process.env['INPUT_CLIENTSECRET']
  const np = process.execPath
  const ip = path.join(__dirname, '..', 'lib', 'main.js')
const result = spawnSync(
  `"${np}"`, [`"${ip}"`],
